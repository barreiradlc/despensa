...
export async function storePantries(pantries: Pantry[]) {
  ...
  for (let pantry of pantries) {
    ...
    const localpantry = realm
      .objects<Pantry[]>('Pantry')
      .filtered('uuid = $0 or _id = $1', uuid, _id)[0];
    ...
    for (let item of pantry.items) {
      const localItem = realm
        .objects<Item>('Item')
        .filtered('uuid = $0 or _id = $1', item.uuid, item._id)[0];
...
export async function editLocalPantry(pantry: Pantry | CreatePantryDTO) {
  try {
    ...
    let localpantry = await realm
      .objects<PantryInterface>('Pantry')
...
export async function getLocalProvision({name, _id}: ProvisionInterface) {
  try {
    ...
    if (name) {
      localProvision = await realm
        .objects<ProvisionInterface>('Provision')
...
export async function createLocalItem(
  item: ItemInterface | CreateItemDTO,
  uuidPantry: string,
) {
  let localpantry = await realm
    .objects<PantryInterface>('Pantry')
    .filtered('uuid = $0', uuidPantry)[0];
...
export async function updateLocalItem(item: ItemInterface | CreateItemDTO) {
  let localItem = await realm
    .objects<ItemInterface>('Item')
    .filtered('uuid = $0', item.uuid)[0];
